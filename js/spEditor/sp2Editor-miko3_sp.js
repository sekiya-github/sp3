...
// コンソールにて確認：document.getElementById('htmlBody_ifr').contentDocument.querySelector('.spEdit')
// コンソールにて確認：document.getElementById('sp-js-template').content.querySelector('#' + dragId);
(function($) {
	...
	$.fn.sp2Editor = function(options) {
		...
		contentWidndow.onload = function(){
			...
			fdoc.find('html').on({
				...
				'paste': function(e) {
					...
					if (spEdit.length > 0) {
						...
						//===============================================
						// データ取得　jquery eventにclipboardDataはない：http://manji602.hatenablog.com/entry/20130829/1377767902
						//===============================================
						...
						// 書式設定：なし
						if(t.hasClass('spEdit') > 0){
							cc('paste:書式設定なし');
							cData = cData.replace(/\r?\n/g, '<br>');
							contentDocument.execCommand('insertHtml', false, cData);
						...
						// 書式設定；あり
							...
							// 以下をすると貼り付けはうまくいくが、redoで__BR__が見えてしまう。かつ2回余分に戻らないといけない。
							// なので、書式設定ありの(ほとんどの場合<span>で囲まれた)場合、改行なしで貼り付けする。
							//cData = cData.replace(/\r?\n/g, '__BR__');
							//contentDocument.execCommand('insertText', false, cData);
							
							//var spEdit = fdoc.find('.spEdit');
							//var newHTML = replaceAll(spEdit.html(),'__BR__','<br>');
							//
							//contentDocument.execCommand('selectAll', false, null);
							//contentDocument.execCommand('insertHtml', false, newHTML);
						}
				...
				'dragover': function(e) {
					...
					if (!t.hasClass('sp-drop-here') && t.closest('.sp-drop-here').length == 0) {
						...
						if (dragType == 'item') {
							...
							if (t.hasClass('sp-item')) {
								...
								if (t.find('.sp-item').length == 0) {
									...
									//f_replaceDragItem(e, t); // box内にsp-itemがある場合、sp-item:hoverに任せる
								}
						...
						//===============================================
							...
							if (t.hasClass('sp-item') && t.closest('.sp-box').length == 0) {
								//cc('sp-item');
								f_replaceDragItem(e, t);

							} else if (t.closest('.sp-item').length > 0 && t.closest('.sp-box').length == 0) {
								//cc('sp-item(ct)');
								t = t.closest('.sp-item').first();
								f_replaceDragItem(e, t);
			...
			//-----------------------------------------------
			fdoc.find('html','#ipk-menu-dra').on({
...
//fdoc.find('body').css('zoom','0.2');
					...
					//cc(dragIcon);
					//cc(e.originalEvent.dataTransfer);
					e.originalEvent.dataTransfer.effectAllowed = 'move';
					e.originalEvent.dataTransfer.dropEffect = 'move';


					var dragIcon = document.createElement('img');
					dragIcon.src = 'img_ph/d48b10db-video-icon_00p00p00p00p000000.png';
					dragIcon.width = 100;
					dragIcon.style.zIndex = 999;
					dragIcon = document.getElementById('htmlBody_ifr').contentDocument.querySelector('#phimg');
					e.originalEvent.dataTransfer.setDragImage(dragIcon, 10, 10);

					dragFlag = true;
					dragObj = fdoc.find('.spEdit').first();
					dragHeight = dragObj.addClass('sp-drop-here').height();
					dragHtml = dragObj.addClass('sp-drop-here').prop('outerHTML');
					dragType = '';
					if (dragObj.hasClass('sp-item')) {
						dragType = 'item';
						fdoc.find('body').addClass('sp-highlight-item').removeClass('sp-highlight-section sp-highlight-row sp-highlight-col');
					} else if (dragObj.hasClass('sp-box')) {
						dragType = 'box';
						fdoc.find('body').addClass('sp-highlight-item').removeClass('sp-highlight-section sp-highlight-row sp-highlight-col');
					} else if (dragObj.hasClass('row')) {
						dragType = 'grid';
						fdoc.find('body').addClass('sp-highlight-col').removeClass('sp-highlight-section sp-highlight-row sp-highlight-item');
					} else if (dragObj.hasClass('sp-section')) {
						dragType = 'section';
						fdoc.find('body').addClass('sp-highlight-section').removeClass('sp-highlight-row sp-highlight-col sp-highlight-item');
					}
...
//fdoc.find('body').css('zoom','1');
					...
					fdoc.find('.spEdit').removeClass('targetOutline').removeClass('spEdit').removeAttr('contenteditable');
					fdoc.find('#ipkMenu').hide();
					
					fdoc.find('body').removeClass('sp-highlight-section sp-highlight-row sp-highlight-col sp-highlight-item');
					
					if($('#checkbox-1').prop('checked')){
						fdoc.find('body').addClass('sp-highlight-section sp-highlight-row sp-highlight-col sp-highlight-item');
					}
					
					var dropHere = fdoc.find('.sp-drop-here');
					dropHere.removeClass('sp-drop-here').addClass('targetOutline').trigger('click');
			...
			// ページIDセット
			fdoc.find('body').data('page-id',pageId);
		...
		$('.sp-js-change-setThemeClass').on('change', f_setThemeClass);
		...
		//-----------------------------------------------
		// dialog : edit countdown
		//-----------------------------------------------
		var countdownDialog = document.getElementById('sp-id-dialog-name-countdown');
		countdownDialog.querySelector('.close').addEventListener('click', function() {
			countdownDialog.close();
		});
		$('#sp-id-countdown-edit-btn').on('click', function() {
			countdownDialog.showModal();
			var spEdit        = fdoc.find('.spEdit').first();
			var deadlineDate  = spEdit.attr('data-deadline-date');
			var deadlineTime  = spEdit.attr('data-deadline-time');
			var designType    = spEdit.attr('data-design-type');
			var countdownCode = jQuery.trim(spEdit.find('template').first().html());
			cc(countdownCode);

			// ID属性がないなら、ID付与
			if(!spEdit.attr('id')){
				spEdit.attr('id','spid'+$.now());
			}
			
			if(countdownCode.length == 0){
				deadlineDate  = addDate(3);
				deadlineTime  = '00:00';
				designType    = 'A';
				countdownCode = document.getElementById('sp-id-template-countdown').content.querySelector('#'+designType).innerHTML;
				countdownCode = replaceAll(countdownCode,'{$id}',spEdit.attr('id'));
				countdownCode = replaceAll(countdownCode,'{$datetime}',deadlineDate+' '+deadlineTime+':00');
				console.log(countdownCode);
			}
			
			$('#sp-id-dialog-input-deadline-date').val(deadlineDate);
			$('#sp-id-dialog-input-deadline-time').val(deadlineTime);
			$('#sp-id-dialog-input-design-type').val(designType);
			$('#sp-id-dialog-input-countdown-code').val(countdownCode);
			
			// iframe作成
			$('#sp-id-dialog-iframe-countdown').html('<iframe id="sp-id-iframe-countdown-list" src=\'javascript:""\' frameborder="0" style="width: 100%; height: 100%; display: block;background-color:white;"></iframe>');
			document.getElementById('sp-id-iframe-countdown-list').contentDocument.write('<div style="width:calc(100% - 50px);margin:35px auto">'+countdownCode+'</div>');
			//document.getElementById('sp-id-iframe-countdown-list').contentDocument.location.hash='#'+designType;
		});
		countdownDialog.querySelector('#sp-id-dialog-input-design-type').addEventListener('change', function() {
			var spEdit        = fdoc.find('.spEdit').first();
			// 現在ダイアログでセットされている値を使用（現在のspEdit値ではない）
			var deadlineDate  = $('#sp-id-dialog-input-deadline-date').val();
			var deadlineTime  = $('#sp-id-dialog-input-deadline-time').val();
			var designType    = $('#sp-id-dialog-input-design-type').val();
			var countdownCode = $('#sp-id-dialog-input-countdown-code').val();

			// 選択したカウントダウンのHTMLを取得
			countdownCode = document.getElementById('sp-id-template-countdown').content.querySelector('#'+designType).innerHTML;
			countdownCode = replaceAll(countdownCode,'{$id}',spEdit.attr('id'));
			countdownCode = replaceAll(countdownCode,'{$datetime}',deadlineDate+' '+deadlineTime);
		...
		countdownDialog.querySelector('.sp-js-click-ok').addEventListener('click', function() {
			...
			spEdit.attr('data-deadline-date', deadlineDate);
			spEdit.attr('data-deadline-time', deadlineTime);
			spEdit.attr('data-design-type'  , designType);
			spEdit.html('<template>' + countdownCode + '</template>'); 
			//fdoc.find('.spEdit').first().find('template').first().html( jQuery.trim(replaceCode) ); // js入れると実行される。
			spEdit.attr('data-title', 'カウントダウンタイマー　：　デッドライン日付='+deadlineDate+' '+deadlineTime+':00　タイプ='+designType);

			countdownDialog.close();

			// 履歴登録
			f_historyRegist('dialog : edit countdown');
		});
		...
		function f_editable(t) {
			...
			if(t.hasClass('spEdit')){
				...
				if(t.prop('tagName') == 'IMG'){
					...
					if(t.data('contents') == 'countdown'){
						$('#sp-id-countdown-edit-btn').trigger('click');
					}else{
						$('#sp-id-replace-item-edit-btn').trigger('click');
        			}
				}
			...
			switch(cmd){
				...
				case 'fontSize':
					if(val > 0){
						...
						fdoc.find('.spEdit').find('span[style*="font-size: -webkit-xxx-large"]').css('font-size', val+'px');
					}
		...
		function f_createGridHtml(no){
			...
			gridHtml += '<div class="row sp-row sp-drop-here">'+"\n";
			for(var i = 0; i < gridColArr.length; i++){
				colNo = gridColArr[i];
				gridHtml += '	<div class="col-sm-'+colNo+' sp-col">'+"\n";
		...
		function f_setAnimation(e){
			...
			// ID属性がないなら、ID付与
			if(!spEdit.attr('id')){
				spEdit.attr('id','sp-id-'+$.now());
			}

			var spEditId = spEdit.attr('id');
			var jsTemplateId = 'sp-ani-template-id-'+spEditId;
			//var ct  = $(e.currentTarget);
			var ct  = $('#sp-id-ani-name'); // 複数要素どれを選択しても同じ処理になるので、currentTargetは使えない。
			var className = ct.val();
			var classPrefix = 'sp-ani'; // changeする箇所が複数あるため、ここで固定。data-prefix設定しない。
			var intervalSecond = $('#sp-id-ani-interval-second').val();
			var tableDisp = $('#sp-id-animation-table').find('tr:gt(0)');
			
			//-----------------------------------------------
			// アニメーション設定をいったんすべてキャンセル　※毎回クラスIDを設定し、削除
			//-----------------------------------------------
			// アニメーションクラスを削除
			removeClassPrefix(spEdit,classPrefix);
			spEdit.removeClass('sp-ani');
			spEdit.removeAttr('data-ani-class-name');
			spEdit.removeAttr('data-ani-interval-second');
			// 要素を更新　アニメーションクラスを削除するだけでは、一度設定したイベントが消えない。
			//var outerHtml = spEdit.prop('outerHTML');
			//spEdit.after(outerHtml);
			//spEdit.remove();
			
			// JS定義を削除
			fdoc.find('#'+jsTemplateId).remove();
			
			//-----------------------------------------------
			// スイッチON処理
			//-----------------------------------------------
			if($('#sp-id-spEdit-animation-switch').prop('checked')){
				
				tableDisp.show();
				
				// 指定クラス名があるなら、JS定義を追加
				if(spEditId.length > 0 && jsTemplateId.length > 0 && className.length > 0 && intervalSecond > 0){

					// クラス設定
					spEdit.addClass(classPrefix).addClass(className).attr('data-ani-name',className).attr('data-ani-interval-second',intervalSecond);
					
					// JS append
					var script = contentDocument.createElement('script');
					script.id = jsTemplateId;
					script.innerHTML = 'sp_setAnimation("'+spEditId+'","'+className+'",' + ( intervalSecond * 1000) + ');';
		...
		function f_setTimer(e){
			...
			var spEditId = spEdit.attr('id');
			var jsTemplateId = 'sp-timer-template-id-'+spEditId;
			var timerSecond = $('#sp-id-timer-second').val();
			var firstAccessOnly = $('#sp-id-timer-first-access-only').val();
			var tableDisp = $('#sp-id-timer-table').find('tr:gt(0)');
			var localStrageKey = 'sp3-pid-'+pageId+'-accessed';
			
			//-----------------------------------------------
			// タイマー設定を一旦すべて削除
			//-----------------------------------------------
			// タイマークラスを削除
			spEdit.removeClass('sp-timer');
			spEdit.removeAttr('data-timer-second');
			spEdit.removeAttr('data-timer-first-access-only');
			...
			// スイッチON処理
			//-----------------------------------------------
			if($('#sp-id-spEdit-timer-switch').prop('checked')){
				
				tableDisp.show();
				
				// 指定クラス名があるなら、JS定義を追加
				if(spEditId.length > 0 && jsTemplateId.length > 0 && timerSecond > 0){
					
					// spEdit設定
					spEdit.addClass('sp-timer').attr('data-timer-second',timerSecond).attr('data-first-access-only',firstAccessOnly);

					// JS定義設定
					var timerTemplate = "";
					timerTemplate += "\n<template id=\""+jsTemplateId+"\">";
					timerTemplate += "\n	<!--sp-timer:start-->";
					timerTemplate += "\n	<style>";
					timerTemplate += "\n		.sp-timer{";
					...
					timerTemplate += "\n	</style>";
					timerTemplate += "\n	<script>";
					timerTemplate += "\n		var pageFirstAccessTimestamp = localStorage.getItem('"+localStrageKey+"');";
					timerTemplate += "\n		if(pageFirstAccessTimestamp != null){";
					timerTemplate += "\n			console.log('first accessed');";
					timerTemplate += "\n			// timer set.";
					timerTemplate += "\n			setTimeout(function(){";
					timerTemplate += "\n				document.getElementById('"+spEditId+"').classList.remove('sp-timer');";
					timerTemplate += "\n			},"+ (timerSecond * 1000) +");";
					timerTemplate += "\n			// first access save.";
					timerTemplate += "\n			localStorage.setItem('"+localStrageKey+"', Date.now());";
					timerTemplate += "\n		}else{"
					timerTemplate += "\n			console.log('second accessed');";
					timerTemplate += "\n		}"
					timerTemplate += "\n	</script>";
					timerTemplate += "\n	<!--sp-timer:end-->";
					timerTemplate += "\n</template>";
					
					//timerTemplate += '<template id="'+jsTemplateId+'">';
					//timerTemplate += 	'<!--sp-timer:start-->';
					//timerTemplate += 	'<style>';
					//timerTemplate += 		'.sp-timer{';
					//timerTemplate += 			'visibility:hidden !important;';
					//timerTemplate += 		'}';
					//timerTemplate += 	'</style>';
					//timerTemplate += 	'<script>';
					//timerTemplate += 		'setTimeout(function(){';
					//timerTemplate += 			'document.getElementById(\''+spEditId+'\').classList.remove(\'sp-timer\');';
					//timerTemplate += 		'},'+ (timerSecond * 1000) +');';
					//timerTemplate += 	'</script>';
					//timerTemplate += 	'<!--sp-timer:end-->';
					//timerTemplate += '</template>';
					
					// bodyに追加
					fdoc.find('body').append(timerTemplate);
				}
			//-----------------------------------------------
			// スイッチOFF処理
			//-----------------------------------------------
		...
		//
		//-----------------------------------------------
		function removeClassPrefix(target,classPrefix){
			regexp = new RegExp('\\b' + classPrefix + '\\S+', 'g');
			// classPrefix で始まるclassをすべて削除
			target.removeClass(function(index, currentClass) {
			    return (currentClass.match(regexp) || []).join(' ');
			});
		}
		...
		//
		//-----------------------------------------------
		function f_setThemeClass(e){

			var className = $(this).val();
			var body = fdoc.find('body');
			
			removeClassPrefix(body,'base-color');
		...
		function f_setSpEditComputed(t,spEdit) {
            ...
            // テキスト : フォント
            //-----------------------------------------------
            //closest('span[style*=font-family]');
            ccc(replaceAll(t.css('font-family'),'"',''));
            
			$('#sp-id-font-name').val(replaceAll(t.css('font-family'),'"',''));
			...
			$('#sp-id-border-radius').val(dpx(spEdit.css('border-radius')));
            ...
            // ギミック：タイマー表示
            //-----------------------------------------------
			if(spEdit.hasClass('sp-timer')){
				$('#sp-id-spEdit-timer-switch').prop("checked", true).parent().addClass('is-checked');
				$('#sp-id-timer-second').val(spEdit.data('timer-second'));
				$('#sp-id-timer-table').find('tr:gt(0)').show();
			}else{
				$('#sp-id-spEdit-timer-switch').prop("checked", false).parent().removeClass('is-checked');
				$('#sp-id-timer-second').val($('#sp-id-timer-second').attr('value'));
				$('#sp-id-timer-table').find('tr:gt(0)').hide();
			}

            //-----------------------------------------------
            // ギミック：アニメーション
            //-----------------------------------------------
			if(spEdit.hasClass('sp-ani')){
				$('#sp-id-spEdit-animation-switch').prop("checked", true).parent().addClass('is-checked');
				$('#sp-id-ani-name').val(spEdit.data('ani-name'));
				$('#sp-id-ani-interval-second').val(spEdit.data('ani-interval-second'));
				$('#sp-id-animation-table').find('tr:gt(0)').show();
			}else{
				$('#sp-id-spEdit-animation-switch').prop("checked", false).parent().removeClass('is-checked');
				$('#sp-id-ani-name').val('sp-ani-bounce');
				$('#sp-id-ani-interval-second').val($('#sp-id-ani-interval-second').attr('value'));
				$('#sp-id-animation-table').find('tr:gt(0)').hide();
			}
...
//            cc(e.keyCode);
		...
		/*
		* https://iwb.jp/javascript-new-date-gettime/#i-2
		*/
